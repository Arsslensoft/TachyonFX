...
%token <Object> T_AUTOLOAD 570      //"autoload (T_AUTOLOAD)"
...
reserved_non_modifiers:
	...
	| T_INSTANCEOF | T_NEW | T_CLONE | T_EXIT | T_AUTOLOAD | T_IF | T_ELSEIF | T_ELSE | T_ENDIF | T_ECHO | T_DO | T_WHILE | T_ENDWHILE
...
function_attributes_def:
	'[' function_attributes ']' function
	| function
	{
	$$.FunctionAttributes = new List<FunctionAttribute>();
	}
	;


function_attribute:
  T_STRING '(' argument_list ')'
	{
	if($$.FunctionAttributes == null)
	  	$$.FunctionAttributes = new List<FunctionAttribute>();

		$$.FunctionAttributes.Add(new FunctionAttribute(@1,new Name($1), new CallSignature($3)  ));
	}
	;

function_declaration_statement:
function_attributes_def returns_ref T_STRING backup_doc_comment '(' parameter_list ')' return_type
	backup_fn_flags enter_scope '{' inner_statement_list '}' exit_scope backup_fn_flags
		{
		var attrs = new List<FunctionAttribute>($1.FunctionAttributes);
		$1.FunctionAttributes=null;
		$$ = _astFactory.Function(@$, attrs, isConditional, $2 == (long)FormalParam.Flags.IsByRef, PhpMemberAttributes.None, $8, 
