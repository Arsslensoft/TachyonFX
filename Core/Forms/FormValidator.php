...
class FormValidator
{
     ...
     * @param array $validators The FormValidator validators
     *
    ...
    public static function is_valid(array $data, array $validators)
    {
        ...
        $fvalidator->validation_rules($validators);

        if ($fvalidator->run($data) === false) {
            return $fvalidator->get_readable_errors(false);
        } else {
            return true;
        }
    }
     ...
     * @return mixed True(boolean) or the array of error messages
     */
    public static function is_valid_array(array $data, array $validators)
    {
        $fvalidator = new self();
        ...
        if ($fvalidator->run($data) === false) {
            return $fvalidator->get_errors_array(false);
        } else {
            return true;
        }
    }
    ...
    public static function filter_input(array $data, array $filters)
    {
        ...
        return $fvalidator->filter($data, $filters);
    }
    ...
    */
     ...
     *
     * See: https://github.com/Wixel/FormValidator/issues/5
