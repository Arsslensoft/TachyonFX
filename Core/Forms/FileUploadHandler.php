...
class FileUploadHandler{
    ...
    public static function factory($destination, $root = false) {
        ...
        return new FileUploadHandler($destination, $root);
    ...
    protected function saveFile() {
        ...
        if(!$this->folder_exist( $this->destination))
    ...
    /**
     * File extension validation callback
     *
     * @param object $object
     */
    protected function checkFileExtension($object) {

        if (!empty($object->extensions)) {
            $ext = pathinfo($object->file['original_filename'], PATHINFO_EXTENSION);

            if (!in_array($ext, $object->mimes)) {

                $object->set_error('File extension not allowed.');
     ...
     * @param array $ext
     */
    public function setAllowedFileExtensions($ext) {

        $this->extensions	= $ext;

        //if mime types is set -> set callback
        $this->callbacks[]	= 'check_file_extension';
    ...
    protected function setFileArray($file) {
        ...
        if (!$this->checkFileArray($file)) {
            ...
            $this->setErrors('Please select file.');
